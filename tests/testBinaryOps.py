...
class TestSalesTaxSample(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open('mysql_setting.txt', 'r') as f:
            ...
            ks_merge.reset()
            ks_merge.addTable("./filehandler/data/Sales.csv","Sales")
    ...
    def test_Binary_Op_Aggregate(self):
        self.assertEqual(12, self.ks_analytics.calculate("Units","6/1/14"))
    ...
    def test_Binary_Op_Multiplication_with_groupby(self):
        self.assertEqual(36, self.ks_analytics.calculateGroupBy("sum(Units)* sum(RoyaltyPrice)",
                                                                "VendorId, ProductType, DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    def test_Binary_Op_Multiplication_with_groupby_date(self):
        self.assertEqual(96, self.ks_analytics.calculateGroupBy("sum(Units)* sum(RoyaltyPrice)",
                                                                " DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    def test_Binary_Op_Addition_without_groupby_per_record(self):
        self.assertEqual(20, self.ks_analytics.calculate("Units + RoyaltyPrice","6/1/14"))
    
    def test_Binary_Op_Addition_with_groupby(self):
        self.assertEqual(20, self.ks_analytics.calculateGroupBy("sum(Units) + sum(RoyaltyPrice)",
                                                                "VendorId, ProductType, DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    ...
    def test_Intertable_Multiplication_with_groupby(self):
        self.assertEqual(2.82, self.ks_analytics.calculateGroupBy("sum(TaxRate)* sum(RoyaltyPrice)",
                                                                "VendorId, ProductType, DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    def test_Intertable_Multiplication_with_groupby_date(self):
        self.assertAlmostEqual(4.95360006, self.ks_analytics.calculateGroupBy("sum(TaxRate*RoyaltyPrice)* sum(RoyaltyPrice)",
                                                                " DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    def test_Intertable_Addition_without_groupby_per_record(self):
        self.assertAlmostEqual(8.3096, self.ks_analytics.calculate("TaxRate + RoyaltyPrice","6/1/14"))
    
    def test_Intertable_Addition_with_groupby(self):
        self.assertAlmostEqual(8.3096, self.ks_analytics.calculateGroupBy("sum(TaxRate) + sum(RoyaltyPrice)",
                                                                "VendorId, ProductType, DownloadDate",
                                                                " DownloadDate= '6/1/14'"))
    ...
    def test_Chained_Intertable(self):
        self.assertEqual(22.14240026473999, self.ks_analytics.calculate("REVENUE_AFTER_TAX","6/1/14"))
